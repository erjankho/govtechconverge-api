...
export async function up(db) {
  ...
  await db.schema.alterTable('messages').addColumn('order', 'integer').execute();
  await db.schema
    .alterTable('messages')
    .addUniqueConstraint('conversation_id_order_unique', ['order', 'conversation_id'])
    .execute();
}
...
export async function down(db) {
  ...
  await db.schema.alterTable('messages').dropConstraint('conversation_id_order_unique').execute();
  await db.schema.alterTable('messages').dropColumn('order').execute();
}
