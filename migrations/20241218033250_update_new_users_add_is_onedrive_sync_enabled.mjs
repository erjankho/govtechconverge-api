...
export async function up(db) {
  ...
  await db.schema
    ...
    .addColumn('is_onedrive_sync_enabled', 'boolean', (col) => col.notNull().defaultTo(false))
    .execute();
}
...
export async function down(db) {
  ...
  await db.schema.alterTable('new_users').dropColumn('is_onedrive_sync_enabled').execute();
}
